...
static int ht_hash(const char* s, int a, int m) {
    ...
    for (int i = 0; i < len; i++) {
        hash += (long)int_pow(a, len - (i+1)) * s[i];
        hash %= m;
    }
    return (int)hash;
}

// index = (hash_a(string) + i * (hash_b(string) + 1)) % num_buckets    Handling collisions
...
void ht_insert(ht_hash_table* ht, const char* key, const char* value) {
    ...
    int i = 0;
    while (cur_item != NULL) {
        if (cur_item != &HT_DELETED_ITEM && strcmp(cur_item->key, key)) {
