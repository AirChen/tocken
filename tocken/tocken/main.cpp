...
void process(WFHttpTask *server_task)
{
    ...
    {
        if (name == "sentence") {
            StringUtil::url_decode(value);
            std::vector<std::wstring> res = g_tocken._cut_DAG_NO_HMM(utils::s2ws(value));
            std::string tockenStr;
            for (auto tocken : res) {
                tockenStr.push_back(' ');
                tockenStr.append(utils::ws2s(tocken));
            }
            len = snprintf(buf, 8192, "<p>%s: %s</p>", name.c_str(), tockenStr.c_str());
            resp->append_output_body(buf, len);
        } else {
